...
public:
    ...
    constexpr MontgomeryModInt32& operator+=(const MontgomeryModInt32& rhs) { return v += rhs.v - (P << 1), v += P << 1 & -(v >> 31), *this; }
    constexpr MontgomeryModInt32& operator-=(const MontgomeryModInt32& rhs) { return v -= rhs.v, v += P << 1 & -(v >> 31), *this; }
    constexpr MontgomeryModInt32& operator*=(const MontgomeryModInt32& rhs) { return v = reduce(u64(v) * rhs.v), *this; }
    constexpr MontgomeryModInt32& operator/=(const MontgomeryModInt32& rhs) { return this->operator*=(rhs.inv()); }
    friend MontgomeryModInt32 operator+(const MontgomeryModInt32& lhs, const MontgomeryModInt32& rhs) { return MontgomeryModInt32(lhs) += rhs; }
    friend MontgomeryModInt32 operator-(const MontgomeryModInt32& lhs, const MontgomeryModInt32& rhs) { return MontgomeryModInt32(lhs) -= rhs; }
    friend MontgomeryModInt32 operator*(const MontgomeryModInt32& lhs, const MontgomeryModInt32& rhs) { return MontgomeryModInt32(lhs) *= rhs; }
    friend MontgomeryModInt32 operator/(const MontgomeryModInt32& lhs, const MontgomeryModInt32& rhs) { return MontgomeryModInt32(lhs) /= rhs; }
    friend constexpr bool operator==(const MontgomeryModInt32& lhs, const MontgomeryModInt32& rhs) { return norm(lhs.v) == norm(rhs.v); }
    friend constexpr strong_ordering operator<=>(const MontgomeryModInt32& lhs, const MontgomeryModInt32& rhs) { return norm(lhs.v) <=> norm(rhs.v); }
    friend istream& operator>>(istream& is, MontgomeryModInt32& rhs) { return is >> rhs.v, rhs.v = reduce(u64(rhs.v) * r2), is; }
    friend ostream& operator<<(ostream& os, const MontgomeryModInt32& rhs) { return os << rhs.get(); }

    constexpr MontgomeryModInt32 pow(i64 y) const {
        if ((y %= P - 1) < 0) { // phi(P) = P - 1, assume P is a prime number
            y += P - 1;
        }
...
template <uint32_t P>
MontgomeryModInt32<P> sqrt(const MontgomeryModInt32<P>& x) {
    ...
    static uniform_int_distribution<uint32_t> dis(1, P - 1);
    ...
    public:
        ...
        Field_P2& operator*=(const Field_P2& rhs) {
            value_type tmp1(b * rhs.b), tmp2(a * rhs.a - tmp1 * ax), tmp3(a * rhs.b + b * rhs.a + tmp1 * t);
            a = tmp2, b = tmp3;
            return *this;
        }
        Field_P2 pow(uint64_t y) const {
...
uint64_t get_len(uint64_t n) { // if n=0, boom
...
public:
    ...
    static void dft(i32 n, value_type x[]) {
        ...
        if (lim == 0) {
            ...
            for (i32 i = 18; i != -1; --i) {
                ROOT[1 << i] = ROOT[1 << (i + 1)] * ROOT[1 << (i + 1)], IROOT[1 << i] = IROOT[1 << (i + 1)] * IROOT[1 << (i + 1)];
            }
            lim = 1;
        }
        while ((lim << 1) < n) {
...
template <uint32_t P>
void dft(uint32_t n, MontgomeryModInt32<P> x[]) {
    NTT<P>::dft(n, x);
}

template <uint32_t P>
void idft(uint32_t n, MontgomeryModInt32<P> x[]) {
    NTT<P>::idft(n, x);
}

template <uint32_t P>
vector<MontgomeryModInt32<P>> add(const vector<MontgomeryModInt32<P>>& x, const vector<MontgomeryModInt32<P>>& y) {
    using i32 = int32_t;
    using value_type = MontgomeryModInt32<P>;
    vector<value_type> res(std::max(x.size(), y.size()));
...
template <uint32_t P>
vector<MontgomeryModInt32<P>> sub(const vector<MontgomeryModInt32<P>>& x, const vector<MontgomeryModInt32<P>>& y) {
    using i32 = int32_t;
    using value_type = MontgomeryModInt32<P>;
    vector<value_type> res(std::max(x.size(), y.size()));
    for (i32 i = 0, e = std::min(x.size(), y.size()); i != e; ++i) {
...
template <uint32_t P>
vector<MontgomeryModInt32<P>> mul(const vector<MontgomeryModInt32<P>>& x, const vector<MontgomeryModInt32<P>>& y) {
    using i32 = int32_t;
    using value_type = MontgomeryModInt32<P>;
    static value_type a[1 << 21], b[1 << 21];
    vector<value_type> res(x.size() + y.size() - 1);
...
template <uint32_t P>
vector<MontgomeryModInt32<P>> inv(vector<MontgomeryModInt32<P>> x) {
    assert(!x.empty());
    using value_type = MontgomeryModInt32<P>;
    using i32 = int32_t;
    static value_type a[1 << 21], b[1 << 21];
...
template <uint32_t P>
vector<MontgomeryModInt32<P>> quo(vector<MontgomeryModInt32<P>> x, vector<MontgomeryModInt32<P>> y) {
    assert(!x.empty()), assert(!y.empty());
    if (x.size() == 1) {
        return {x[0] * y[0].inv()};
    }
    using value_type = MontgomeryModInt32<P>;
    ...
    vector<value_type> g0 = inv(vector<value_type>(y.begin(), y.begin() + (len >> 1))), res(n);
...
template <uint32_t P>
vector<MontgomeryModInt32<P>> quorem(vector<MontgomeryModInt32<P>> x, vector<MontgomeryModInt32<P>> y) {
    using i32 = int32_t;
    using value_type = MontgomeryModInt32<P>;
    if (x.size() < y.size()) {
        return {0};
    ...
    x.resize(n);
    vector<value_type> res = quo(x, y);
    std::reverse(res.begin(), res.end());
    return res;
}

template <uint32_t P>
pair<vector<MontgomeryModInt32<P>>, vector<MontgomeryModInt32<P>>> quo_with_rem(const vector<MontgomeryModInt32<P>>& x, const vector<MontgomeryModInt32<P>>& y) {
    using i32 = int32_t;
    using value_type = MontgomeryModInt32<P>;
    static value_type a[1 << 21], b[1 << 21];
    if (x.size() < y.size()) {
    ...
    vector<value_type> q = quorem(x, y);
    if (y.size() == 1) {
        return {q, {0}};
    }
    ...
    idft(len, a);
    vector<value_type> rem(y.size() - 1);
    if (overlap > 0) {
...
template <uint32_t P>
vector<MontgomeryModInt32<P>> rem(const vector<MontgomeryModInt32<P>>& x, const vector<MontgomeryModInt32<P>>& y) {
    return quo_with_rem(x, y).second;
}

template <uint32_t P>
vector<MontgomeryModInt32<P>> deriv(const vector<MontgomeryModInt32<P>>& x) {
    using i32 = int32_t;
    using value_type = MontgomeryModInt32<P>;
    if (x.size() == 1) {
        return {0};
    }
    vector<value_type> res(x.size() - 1);
...
template <uint32_t P>
vector<MontgomeryModInt32<P>> integr(const vector<MontgomeryModInt32<P>>& x) {
    using i32 = int32_t;
    using value_type = MontgomeryModInt32<P>;
    static i32 lim = 0;
    static value_type INV[1 << 21];
    i32 len = x.size() + 1;
    vector<value_type> res(len);
    res[0] = 0;
    if (lim < len) {
        INV[1] = 1;
...
template <uint32_t P>
vector<MontgomeryModInt32<P>> log(const vector<MontgomeryModInt32<P>>& x) {
    return x.size() == 1 ? vector<MontgomeryModInt32<P>> {0} : integr(quo(deriv(x), x));
}

template <uint32_t P>
vector<MontgomeryModInt32<P>> exp(vector<MontgomeryModInt32<P>> x) {
    assert(x[0] == 0);
    using i32 = int32_t;
    using value_type = MontgomeryModInt32<P>;
    i32 n = x.size(), len = get_len(n);
    ...
    static value_type a[1 << 21], b[1 << 21], c[1 << 21], d[1 << 21], e[1 << 21], f[1 << 21], INV[1 << 21];
    static i32 lim = 0;
    if (lim < len) {
        INV[1] = 1;
        for (i32 i = (lim < 2 ? 2 : lim); i < len; ++i) {
    ...
    for (i32 i = 1; i != len; ++i) { // b->f'
        b[i - 1] = value_type(i) * x[i];
    }
...
template <uint32_t P>
vector<MontgomeryModInt32<P>> sqrt(vector<MontgomeryModInt32<P>> x) {
    using i32 = int32_t;
    using value_type = MontgomeryModInt32<P>;
...
template <uint32_t P>
vector<MontgomeryModInt32<P>> sqrtinv(vector<MontgomeryModInt32<P>> x) {
    using i32 = int32_t;
    using value_type = MontgomeryModInt32<P>;
    static value_type a[1 << 21], b[1 << 21], c[1 << 21], d[1 << 21];
    static constexpr value_type iv2(P - ((P - 1) >> 1)), ZERO(0);
    ...
    return vector<MontgomeryModInt32<P>>(a, a + n);
}

template <uint32_t P>
vector<MontgomeryModInt32<P>> pow(const vector<MontgomeryModInt32<P>>& x, const MontgomeryModInt32<P>& k) {
    vector<MontgomeryModInt32<P>> y(log(x));
    for (auto& i : y) {
        i *= k;
    }
    return exp(y);
...
template <uint32_t P>
vector<MontgomeryModInt32<P>> operator+(const vector<MontgomeryModInt32<P>>& x, const vector<MontgomeryModInt32<P>>& y) {
    return add(x, y);
}
template <uint32_t P>
vector<MontgomeryModInt32<P>> operator-(const vector<MontgomeryModInt32<P>>& x, const vector<MontgomeryModInt32<P>>& y) {
