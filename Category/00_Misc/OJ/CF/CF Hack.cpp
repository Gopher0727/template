// Author：名为溶菌酶的溶酶菌
// Link：https://www.acwing.com/blog/content/35391/
// Source: https://codeforces.com/contest/1472/hacks/694823


// 如果有人拿 Java8 或者 Java11，开 int 数组同时使用了 array.sort，那么可以 hack，integer 不行，Java17 不行

template <class T1, class T2>
struct CasePointer {
    uintptr_t value = reinterpret_cast<uintptr_t>((T1 *)nullptr);

    T1 *operator()() const { return reinterpret_cast<T1 *>(value); }

    T1 &operator*() const { return *operator()(); }

    T1 *operator->() const { return operator()(); }

    bool isOther() const { return (value & 1) != 0; }

    T2 *other() { return reinterpret_cast<T2 *>(value & ~uintptr_t(1)); }

    const T2 *other() const { return reinterpret_cast<const T2 *>(value & ~uintptr_t(1)); }

    void operator=(T1 *other) { value = reinterpret_cast<uintptr_t>(other); }

    void operator=(T2 *other) { value = reinterpret_cast<uintptr_t>(other) + 1; }

    bool operator==(const CasePointer &other) const { return value == other.value; }
...
class CartesianList {
    struct Node {
        ...
        Node(const T &value, uint32_t weight) : value(value), weight(weight) {}

        Node(T &&value, uint32_t weight) : value(std::move(value)), weight(weight) {}

        bool hasParent() const { return !parent.isOther(); }
    };
    ...
    public:
        ...
        IteratorImpl(Node *node) { pos = node; }

        IteratorImpl(CartesianList *rootObject) { pos = rootObject; }
        ...
        std::conditional_t<isConst, const T &, T &> operator*() const { return pos->value; }
        ...
        IteratorImpl &operator-=(int32_t value) { return *this += -value; }
    ...
    Node *createNode(const T &value) { return new Node(value, uint32_t(rnd())); }

    Node *createNode(T &&value) { return new Node(std::move(value), uint32_t(rnd())); }
...
public:
    ...
    Iterator begin() { return root ? Iterator(getFirstVertex(root)) : Iterator(this); }

    ConstIterator begin() const {
        return root ? ConstIterator(getFirstVertex(root))
                    : ConstIterator(const_cast<CartesianList *>(this));
    }

    Iterator end() { return Iterator(this); }

    ConstIterator end() const { return ConstIterator(const_cast<CartesianList *>(this)); }
    ...
    Iterator get(uint32_t index) { return Iterator(this, index); }
    ...
    void push_back(const T &value) { push_back_node(createNode(value)); }

    void insert(Iterator pos, const T &value) { insert(pos, createNode(value)); }

    void insert(ConstIterator pos, const T &value) { insert(pos, createNode(value)); }
    ...
    void erase(IteratorImpl<isConst> it) {
        ...
        while (cur->left != nullptr || cur->right != nullptr) {
            if (cur->left != nullptr &&
                (cur->right == nullptr || cur->left->weight >= cur->right->weight)) {
                rotateRight(cur);
            } else {
                rotateLeft(cur);
            }
